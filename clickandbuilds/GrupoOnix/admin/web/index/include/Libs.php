...
class Libs extends Common {
		...
		if(isset($_POST['id'])){
			try{
				...
				foreach ($contenidos as $contenido) {
					if($contenido['seccion'] == 'home_banner_img') {
						$json['home_banner_img'] = '<img src="'.$ruta.'img/'.$contenido['contenido'].'" class="img-fluid">';
					} else {
	...
	function saveRecord() {
		...
		$obligatorios = array("home_banner_txt",
							  "home_compania_txt",
							  "home_productos_txt");
		$excepciones = array("home_banner_img");
		...
		if(!$json["error"]) {
			...
			$sql = "UPDATE contenido SET contenido = ?
					WHERE id = 2";
			$values = array($_POST['home_banner_txt']);
			...
			try {
				...
				$sql = "UPDATE contenido SET contenido = ?
						WHERE id = 3";
				$values = array($_POST['home_compania_txt']);
				$consulta = $db->prepare($sql);
				$consulta->execute($values);

				$sql = "UPDATE contenido SET contenido = ?
						WHERE id = 4";
				$values = array($_POST['home_productos_txt']);
				$consulta = $db->prepare($sql);
				$consulta->execute($values);



				if(isset($_FILES['home_banner_img']['name']) && $_FILES['home_banner_img']['name'] != "") {
					$filename = $_FILES['home_banner_img']['name'];
					$ext = pathinfo($filename, PATHINFO_EXTENSION);
					$name = uniqid();
					$img_name = $ruta."../img/banners/".$name.".".$ext;

					//Redimensiona la imagen
                    list($orig_width, $orig_height) = getimagesize($_FILES[ 'home_banner_img' ][ 'tmp_name' ]);
                    ...
                    $tipofoto =  $_FILES[ 'home_banner_img' ]['type'];
                    if ($tipofoto == 'image/jpg' || $tipofoto == 'image/jpeg') {
                        $image = imagecreatefromjpeg($_FILES[ 'home_banner_img' ][ 'tmp_name' ]);
                    }

                    if ($tipofoto == 'image/png') {
                       $image = imagecreatefrompng($_FILES[ 'home_banner_img' ][ 'tmp_name' ]);
                    }
