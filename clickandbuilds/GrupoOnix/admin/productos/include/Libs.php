...
class Libs extends Common {
		...
		try {
			...
			*/
			...
			foreach ($puntero as $producto) {
				...
				$aRow = array($producto["nombre"], $producto["descripcion"], $img ,$btn_editar.$btn_borrar);
	...
	function deleteRecord() {
		...
		if(isset($_POST['id'])){
			try{
				$consulta = $this->_conexion->prepare("DELETE FROM productos WHERE id = :valor");
				$consulta->bindParam(':valor', $_POST['id']);
				$consulta->execute();
				if($consulta->rowCount()){
					$json['msg'] = "El producto fue eliminado con éxito.";
					$json['error'] = false;
				} else{
					$json['error'] = true;
					$json['msg'] = "El producto elegido no pudo ser eliminado.";
				}
			}catch(PDOException $e){
				die($e->getMessage());
			}	
		...
		if(!$json["error"]) {
			...
			try {
				...
				if(isset($_FILES['imagen']['name']) && $_FILES['imagen']['name'] != "") {
					...
					$name = uniqid();
					$img_name = $ruta."../img/productos/".$name.".".$ext;
					...
					/*if(!move_uploaded_file($_FILES["img-principal"]["tmp_name"], $img_name)){
						...
						$sql_img = "UPDATE productos SET imagen = ?
								    WHERE id = ?";
						
						$values_img = array('productos/'.$name.".".$ext, $pro_id);
			...
			$db->commit();
			$json['msg'] = 'El producto se guardó con éxito.';
		}
