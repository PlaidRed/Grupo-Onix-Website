...
//$baseUrl = $_SERVER["SERVER_NAME"]."/demos/onix/admin/";
$baseUrl = $_SERVER["SERVER_NAME"]."/admin/";
	...
	class Core extends Database {
		...
		/*DATOS ENVIO DE MAILS*/
	   	/*public $smtp_url = 'a2plcpnl0103.prod.iad2.secureserver.net';
	    public $puerto = 465;
	    public $username = 'no-reply@rower.mx';
		...
		public function encrypt($string) {
			...
			switch ($this->classParams["salt"]) {
				...
				case 'md5':
					...
					break;
				default:
					return crypt($string, '$2y$05$defaultSaltForBlowfish!!');
					break;
			}
		 ...
		 */
		public function printButton($module, $type, $params = null, $icon = null, $color = null) {
				...
				switch ($type) {
					case 'alta':
						if ($permissions[$module][1] == 1) {
							...
							$boton = '<a class="'.$classes.' btn-action" href="'.$link.'" data-id="'.$data_id.'" '.$extras.' ><button type="button" class="btn '.$color.'"><i class="fa '.$icon.'"></i>'.$title.'</button></a>';
						}
						break;
					...
					case 'baja':
						if ($permissions[$module][2] == 1) {
							...
							$boton = '<a class="'.$classes.' btn-action" href="'.$link.'" data-id="'.$data_id.'" '.$extras.' ><button class="btn '.$color.' type="button"><i class="fa '.$icon.'"></i>'.$title.'</button></a>';
						}
						break;
		...
		public function sentinel($module, $destiny=''){
			...
			$_SESSION["onx"]["userid"] = 1; //Fake a logged-in user
			
			header("Location: home.php");
			exit();
			//QUITAR ANTES DE PRODUCCION
			global $ruta;
			if(isset($_SESSION["onx"]["userid"])) {
		...
		public function getUsersIds ($module) {
		 ...
		 /*
		 * @author: Cynthia Castillo
		 * @version: 0.1 2020-06-17
		 * 
		 * @param '$titulo'			string. Título de la Notificación
		 * @param '$mensaje'		string. Mensaje de la Notificación
		 * @param '$url'			string. URL a donde irán al hacer click
		 * @param '$url_externa'	boolean. 0 = URL interna; 1 = URL externa
		 * @param '$icono'			string. Icono para la notificación
		 * @param '$color'			string. Color de la notificación
		 * 
		 * Método que hace una nueva notificación para todos los usuarios
		 */
		public function newNotification($titulo, $mensaje, $url, $url_externa, $icono, $color) {
			//Obtenemos todos los usuarios
			$sql = "SELECT SIU_ID FROM SISTEMA_USUARIO";
			$consulta = $this->_conexion->prepare($sql);
			$consulta->execute();
			$usuarios = $consulta->fetchAll(PDO::FETCH_ASSOC);
			$usuarios = implode('|', array_column($usuarios, 'SIU_ID'));

			//Guardamos la notificación
			$sql = 'INSERT INTO SISTEMA_NOTIFICACIONES (SIN_MENSAJE, 
														SIN_LIGA,
														SIN_LIGA_EXTERNA,
														SIN_USUARIOS,
														SIN_ICONO,
														SIN_COLOR)
					VALUES ( ?, ?, ?, ?, ?, ? )';
			$values = array($titulo.'<br>'.$mensaje,
							$url,
							$url_externa,
							'|'.$usuarios.'|',
							$icono,
