		...
		try {
			...
			*/
			...
			foreach ($puntero as $memo) {
				...
				//Botones
				$params_editar = array(	"link"		=>	"cambios.php?id=".$memo['id'],
										"title"		=>	"Ver/Editar");
				...
				$aRow = array($fecha,$memo['titulo'], $memo["contenido"] ,$btn_editar.$btn_borrar);
		...
		if(isset($_POST['id'])){
			try{
				...
				if ($consulta->rowCount() > 0) {
					...
					$row['pdf'] = ($row['pdf'] != '' ? '<a href="include/pdf/'.$row['pdf'].'" target="_blank" class="btn btn-info"><i class="la la-file-pdf-o
"></i> Ver PDF Actual</a>' : '');
		...
		if(!$json["error"]) {
			...
			try {
				...
				if(isset($_POST['id'])) {
					...
					$this->newNotification('Nuevo Aviso',
										   $_POST['titulo'],
										   'avisos/aviso.php?id='.$id,
										   0,
										   'ft-message-circle',
										   'info');
				}
