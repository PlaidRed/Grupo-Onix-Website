		...
		function sendMail() {
			...
			if ($json['error'] == false) {
				...
				try {
					...
					//$mail->AddAddress('ca.castilloe@gmail.com', 'Cynthia Castillo');
					//$mail->AddBCC('ca.castilloe@gmail.com', 'Cynthia Castillo');

					$mail->Subject = "Bolsa de Trabajo | Seguros Onix"; 
					...
					$contenido = '<div style="width: 600px; text-align:left;">
								  ...
								  <p><strong>Edad:</strong> '.$_POST['edad'].'</p>
								  <p><strong>E-mail:</strong> '.$_POST['email'].'</p>
								  <p><strong>Tipo de Cédula:</strong> '.$_POST['cedula'].'</p>
								  <p><strong>Teléfono:</strong> '.$_POST['telefono'].'</p>
								  <p><strong>Años de experiencia en seguros:</strong> '.$_POST['experiencia'].'</p>
								  <p><strong>Compañías con las que trabajas:</strong> '.$_POST['companias'].'</p>
								  <p><strong>Ramos que dominas:</strong> '.$_POST['ramos'].'</p>
